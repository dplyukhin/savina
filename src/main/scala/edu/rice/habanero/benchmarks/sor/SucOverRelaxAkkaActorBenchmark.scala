...
object SucOverRelaxAkkaActorBenchmark {
  ...
  private final class SucOverRelaxAkkaActorBenchmark extends Benchmark {
    ...
    def runIteration() {
      ...
      AkkaActorState.startActor(sorRunner)
      ...
      msg match {
        ...
        case SorBorderMessage(mBorder) =>
          ...
          for (i <- 0 until s) {
            for (j <- 0 until part) {
              val pos = i * (part + 1) + j
              sorActors(pos) ! SorStartMessage(SucOverRelaxConfig.JACOBI_NUM_ITER, sorActors)
            }
          }
      }
    }
  ...
  class SorPeer(
    ...
    private def boot(): Unit = {
      ...
      sorSource ! SorBorderMessage(SorBorder(myBorder))
